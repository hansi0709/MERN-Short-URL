...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    const renderTableData = () => {
        if (!data || data.length === 0) {
            return (
                <tr>
                    <td className='px-6 py-3 text-center' colSpan={1}>No data available</td>
                </tr>
            );
        }
        ...
        return data.map((item) => {
            //data - array that you want to iterate over
            //map() - array method in JavaScript - creates a new array by calling a provided functionon every item in the original array
            //item - current element being processed in the array
        ...
        <div className='relative overflow-x-auto shadow-sm sm:rounded-lg '>
            <table className='w-full table-fixed text-sm text-left rtl:text-right text-gray-500'>
                <thead className='text-md uppercase text-gray-50 bg-gray-700'>
                    <tr>
                        <th scope='col' className='px-5 py-3 w-6/12'>
                            FullUrl
                        </th>
                        <th scope='col' className='px-5 py-3 w-3/12'>
                            ShortUrl
                        </th>
